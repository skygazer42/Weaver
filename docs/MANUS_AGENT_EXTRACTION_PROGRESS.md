...
| Manus 工具 | Weaver 对应 | 状态 |
|-----------|------------|------|
| `sb_image_edit_tool.py` | `tools/sandbox_image_edit_tool.py` | ✅ 刚实现 |
| 滤镜 (blur/sharpen/emboss) | apply_image_filter | ✅ 已实现 |
| 效果 (grayscale/sepia/invert) | apply_image_effect | ✅ 已实现 |
| 调整 (亮度/对比度/饱和度) | adjust_image | ✅ 已实现 |
| 旋转/翻转 | rotate_flip_image | ✅ 已实现 |
| 水印 | add_watermark | ✅ 已实现 |
| 图像叠加 | overlay_images | ✅ 已实现 |
| 缩略图 | create_thumbnail | ✅ 已实现 |

### 13. PPT 大纲生成工具 (新增)
| Manus 工具 | Weaver 对应 | 状态 |
|-----------|------------|------|
| `sb_presentation_outline_tool.py` | `tools/sandbox_presentation_outline_tool.py` | ✅ 刚实现 |
| LLM 生成大纲 | generate_presentation_outline | ✅ 已实现 |
| 大纲转 PPT | outline_to_presentation | ✅ 已实现 |
| 优化大纲 | refine_presentation_outline | ✅ 已实现 |
| 展开单页内容 | expand_slide_content | ✅ 已实现 |

### 14. PPT 增强工具 V2 (新增)
| Manus 工具 | Weaver 对应 | 状态 |
|-----------|------------|------|
| `sb_presentation_tool_v2.py` | `tools/sandbox_presentation_tool_v2.py` | ✅ 刚实现 |
| 幻灯片切换效果 | set_slide_transition | ✅ 已实现 |
| 主题/配色方案 | apply_presentation_theme | ✅ 已实现 |
| 背景设置 | set_slide_background | ✅ 已实现 |
| 复制幻灯片 | duplicate_slide | ✅ 已实现 |
| 重排幻灯片 | reorder_slides | ✅ 已实现 |
| 文本框 | add_text_box | ✅ 已实现 |
...
| Manus 工具 | Weaver 对应 | 状态 |
|-----------|------------|------|
| `sb_web_dev_tool.py` | `tools/sandbox/sandbox_web_dev_tool.py` | ✅ 已实现 |
| `sb_deploy_tool.py` | 同上 (deploy 工具) | ✅ 已实现 |
| `sb_expose_tool.py` | 已覆盖（`sandbox_expose_port`） | ✅ 已实现 |
...
### 3. AgentPress 核心 (优先级: 低 - LangGraph 已覆盖)
...
- 增强取消管理
- **电子表格工具** (Excel/CSV 生成) ✨
- **演示文稿工具** (PowerPoint 生成) ✨
- **图像分析工具** (OCR/图像处理) ✨
- **高级图像编辑** (滤镜/特效/水印) ✨ 新增
- **PPT 大纲生成** (LLM 驱动) ✨ 新增
- **PPT 增强 V2** (主题/动画/背景) ✨ 新增

⏳ **可选/待实现**:
- Web 开发工具 (项目脚手架、部署) - 已完成
